...
public class InterceptionStrangerPreference extends CheckBoxPreference {

    private Context mContext;
    private final static String TAG = "InterceptionStrangerPreference";
    private SharedPreferences mPrenferences;

    public InterceptionStrangerPreference(Context context, AttributeSet attrs) {
        super(context, attrs);
        mContext = context;
        prepare();
    }
    ...
    private String getCurrentKey() {
        String currentKey = PreferenceUtils.KEY_STRANGER_ALL;
        if (mPrenferences == null) {
            return currentKey;
        }
        ...
        if (simSet == true) {
            // SIM 1
            if (currentSim == 1) {
                currentKey = PreferenceUtils.KEY_STRANGER_SIM1;
            } else {
                // SIM 2
                currentKey = PreferenceUtils.KEY_STRANGER_SIM2;
            }
        } else {
            // simSet checked state is false
            currentKey = PreferenceUtils.KEY_STRANGER_ALL;
        }
        return currentKey;
    }
